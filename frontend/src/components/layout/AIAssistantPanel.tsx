  ...
  return (
    ...
    >
      ...
      <div className={cn('flex-1 max-w-[85%]', isUser && 'flex flex-col items-end')}>
        {msg.thinking ? (
          <div className="px-3 py-2 rounded-2xl" style={{ background: 'var(--elevated)', border: '1px solid var(--border)' }}>
            <ThinkingDots />
          </div>
        ) : (
          <div
          ...
          >
            ...
            {!isUser && (
              <button
                onClick={copyText}
                className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-[var(--border)]"
              >
        ...
        <span className="text-[10px] text-[var(--text-4)] mt-1 px-1">
          {msg.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
          {copied && <span className="ml-2 text-cyan-400">Copied!</span>}
        </span>
      </div>
    </motion.div>
  )
      ...
      {aiPanelOpen && (
        <>
          ...
          >
            ...
            {/* Input */}
            <div
              className="px-4 pb-4 flex-shrink-0 border-t border-[var(--border)] pt-3"
            >
              <div
